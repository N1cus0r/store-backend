    ...
    def get(self, request):
        ...
        if all((category, brand, slug)):
            related_products = Product.objects.filter(category=category,
                                                    brand=brand).exclude(slug=slug)[:12]     
            serializer = ProductSerializer(related_products, many=True) 
            
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response({'error': 'Query params were not provided'},
...
class GetLatestProducts(APIView):
    def get(self, request):
        ...
        if category:
            ...
            return Response(serializer.data, status=status.HTTP_200_OK)            
        return Response({'error': 'Category was not provided in params'}, 
                        status=status.HTTP_400_BAD_REQUEST)            
...
class GetFilteredProducts(APIView, FilteredProductsPagination):
    def get(self, request):
        ...
        return Response({'error': 'Query filter were not provided in the params'}, status=status.HTTP_400_BAD_REQUEST)
